...
def get_emotion(recording, text):
    set_cpu_only()
    #keep the order of emotions, but you can rename them to any other form of the word e.g. happiness, do not remove any
    emotions_to_predict = ['Happy', 'Sad', 'Angry', 'Surprised', 'Disgust', 'Fear', 'other']
    #thresholds set during validation
    thresholds = [0.4, 0.25, 0.2, 0.1, 0.25, 0.1, 0.1]
    model = get_model(emotions_to_predict, model_path='/home/ccys/SATbot2.0/model/speech_emotion/multilabel7.pth', model_type='multilabel')
    model, audio, token_ids, attention_mask = load_data(model, audiofile=recording, text=text, model_type='multilabel')
    if audio is None:
        return 'Noemo'
    else:
        prediction = predict(model, audio, token_ids, attention_mask)
        pred_emotion = process_pred(prediction, emotions_to_predict, thresholds=thresholds, model_type='multilabel')
...
"""
def get_model(emotions, model_path='/home/ccys/SATbot2.0/model/speech_emotion/Framework/emotion_recognition_tool/best_model_cpu.pth', model_type='multilabel'):
...
"""
def load_data(model, audiofile, text = None, model_type='multilabel'):
...
"""
def process_pred(prediction, emotions, thresholds=[0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5], model_type='multilabel'):
    if model_type == 'multiclass':
...
"""
def predict(model, audio, token_ids, attention_mask):
    model.eval()
    with torch.no_grad():
