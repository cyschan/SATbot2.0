...
class EmotionRecognitionModel(nn.Module):
    ...
    def forward(self, audio, token_ids, attention_mask, participants=None, hx=None):
        ...
        #nlp_out = torch.utils.checkpoint.checkpoint(self.nlp_model, audio, token_ids, attention_mask)
        ...
        if self.predict_emotion:
            ...
            batch = out.size(0)
            hidden = out.size(1)
            emotion = out.view(1, batch, hidden)
        ...
        if self.predict_details:
            arousal = self.arousal_layer(out)
            arousal = self.arousal_head(arousal)
            valence = self.arousal_layer(out)
            valence = self.valence_head(valence)
            dominance = self.arousal_layer(out)
