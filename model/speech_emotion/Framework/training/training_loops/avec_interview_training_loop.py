    ...
    def train_sub_groups(self, utterance_loader, participants, labels, sub_batch_size=8):
        ...
        for j, sample in enumerate(utterance_loader):
            ...
            try:
                ...
                print_cuda_stats('Failed to process hidden state, clearing cache and skipping')
                torch.cuda.empty_cache()
                print_cuda_stats('Skipping utterance {}'.format(j))
                continue
            ...
            if self.emotion_model:
                ...
                emotion_logits.append(flat_logits)
